...
var pathwayLeads = mysqlTable("pathwayLeads", {
  id: int("id").autoincrement().primaryKey(),
  ...
  destination: varchar("destination", { length: 72 }).notNull(),
  stage: varchar("stage", { length: 72 }).notNull(),
  recommendedRoute: varchar("recommendedRoute", { length: 72 }).notNull(),
  consentToContact: boolean("consentToContact").notNull().default(false),
  createdAt: timestamp("createdAt").defaultNow().notNull()
});
...
async function createPathwayLead(lead) {
  const db = await getDb();
  if (!db) {
    throw new Error("Lead capture is temporarily unavailable");
  }
  const result = await db.insert(pathwayLeads).values(lead);
...
var pathfinderPersonas = ["learner", "parent", "institution", "government"];
var pathfinderDestinations = ["global-schooling", "alternative-pathways", "international-curriculum", "ai-learning", "future-skills", "institutional-transformation"];
var pathfinderStages = ["exploring", "ready-to-start", "expanding-an-offering", "national-programme"];
...
function recommendPath(destination, persona) {
...
// server/routers.ts
var pathfinderSubmission = z2.object({
  firstName: z2.string().trim().min(1, "Please enter your first name").max(120),
  email: z2.string().trim().email("Please enter a valid email address").max(320),
  organization: z2.string().trim().max(180).optional(),
  persona: z2.enum(pathfinderPersonas),
  destination: z2.enum(pathfinderDestinations),
  stage: z2.enum(pathfinderStages),
  consentToContact: z2.literal(true, { error: "Please confirm that AetherMind may contact you" })
});
...
function serveStatic(app) {
  const compiledServerDir = path2.dirname(process.argv[1] ?? "");
  const distPath = path2.resolve(compiledServerDir, "public");
...
async function startServer() {
  ...
  const runningCompiledServer = /[\\/]dist[\\/]index\.js$/.test(process.argv[1] ?? "");
  const isDevelopment = process.env.NODE_ENV === "development" && !runningCompiledServer;
